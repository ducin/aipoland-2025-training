...
Interactive terminal-based chat application with multi-LLM support.
...
- **Multi-LLM Backend:** Support for Google Gemini, Anthropic Claude, OpenAI GPT, and local LLaMA models
...
**Note:** Each engine uses its own model name variable (`GEMINI_MODEL_NAME`, `ANTHROPIC_MODEL_NAME`, `OPENAI_MODEL_NAME`). You can configure all engines in your `.env` file and switch between them by changing the `ENGINE` variable.
...
4. **LLM Layer:** Abstract LLM client interface with multiple implementations (Gemini, Anthropic, OpenAI, LLaMA)
...
- **OpenAILLMClient:** OpenAI GPT API integration
